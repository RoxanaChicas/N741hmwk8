...
All variables cluster together except mitosis.
...
Mitosis is in the cluster with Malignant but not within the cluster of the benign subset.
Malignant dataset is more scattered showing more variability.  Clumpthickness and barenucfix are further away from the other clusters in Malignant.  Uniform cell size and uniform cell shape appear to cluster together on both plots.
...
For the overall dataset using 2 princiapl components it visually summerizes the variability well. The overall data bar plot of the variances shows most of the variance is found within component 1.  

For the benign data set component 1 represents the majority of the variance as in the overall dataset. 
The other components seems to increase in variance but the majority of variance remains within component 1 as in the overall dataset. 2 components seam reasonable for summariing the variability. 
 
For the malignant dataset components 1,2, and 3 show more variance.  In this dataset a third axis may show different patterns of clustering or relationships.  
...
Yes, 2 PCs for both benign and malignant datasets show different clusters and variability from each other.
